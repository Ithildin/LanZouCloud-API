...
class LanZouCloud(object):
    ...
    def _get(self, url, **kwargs):
        try:
            return self._session.get(url, proxies=self._proxy, headers=self._headers, verify=False, timeout=self._timeout, **kwargs)
    ...
    def _post(self, url, data, **kwargs):
        try:
            if 'headers' in kwargs.keys():  # 上传文件需要重新设置 headers，防止重复冲突
                return self._session.post(url, data, proxies=self._proxy, verify=False, timeout=self._timeout, **kwargs)
            return self._session.post(url, data, proxies=self._proxy, headers=self._headers, verify=False, timeout=self._timeout, **kwargs)
        except (ConnectionError, requests.RequestException):
            return None
